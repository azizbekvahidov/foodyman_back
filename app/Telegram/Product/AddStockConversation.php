...
class AddStockConversation extends Conversation
{
...
//                ->where('locale', $locale)->orWhere('locale', $locale),
    ...
    public function collectData(Stock|null $stock): array
    {
        ...
        $quantity = $this->type === 'minus' ? ($cartDetail?->quantity ?? 0) - 1 : ($cartDetail?->quantity ?? 0) + 1;
        $quantity = max($this->actualQuantity($stock, $quantity) ?? 0, 0);
