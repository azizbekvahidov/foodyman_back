...
namespace App\Services\Booking\ShopBookingClosedDateService;
...
class ShopBookingClosedDateService extends CoreService
{
    ...
    public function create(array $data): array
    {
        try {
            ...
            foreach (data_get($data, 'dates', []) as $date) {
                ...
                $exist = ShopBookingClosedDate::where([
    ...
    public function update(int $shopId, array $data): array
    {
        try {
            ...
            BookingShop::find($shopId)->bookingClosedDates()->delete();

            $dates = data_get($data, 'dates');

            foreach (is_array($dates) ? $dates : []  as $date) {

                ShopBookingClosedDate::create(['shop_id' => $shopId, 'date' => $date]);
    ...
    public function delete(?array $ids = [], ?int $shopId = null) {
        ...
        $shopClosedDates = ShopBookingClosedDate::when($shopId, fn($q, $shopId) => $q->where('shop_id', $shopId))
