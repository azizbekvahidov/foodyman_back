...
class ParcelOrderController extends AdminBaseController
{
    ...
    public function store(StoreRequest $request): JsonResponse
    {
        ...
        if ((int)data_get(Settings::adminSettings()->where('key', 'parcel_order_auto_approved')->first(), 'value') === 1) {
            $validated['status'] = ParcelOrder::STATUS_ACCEPTED;
        }

        $check = Utility::pointInPolygon($request->input('address_from'), $request->input('address_to'));
